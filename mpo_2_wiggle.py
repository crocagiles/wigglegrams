...
def mpo_2_vid_gif(mpo_file, overwrite=False, roi_select=True):
    ...
    mp4_out_path = dir_new.parent / (dir_new.name + '_wiggle.mp4')

    if overwrite:
        if mp4_out_path.exists():
            print(f'Skipping {mp4_out_path.name}')
            return
    ...
    # adjust wb and gain of right image (generated by whitebalance.py)
    # This can be commented out if you don't want to calibrate your camera
    image2 = image2 * 0.960473
    image2[:,:,0], image2[:,:,2] = image2[:,:,0]*0.98201906, image2[:,:,2]*0.98611152
    # image1[image1 < 0] = 1.0
    ...
    create_video_from_images(clipped, str(mp4_out_path), crop_ind=[x, y, width, height])
    print(f'video with {len(frames)} frames')
    # media.show_video(frames, fps=30, title='FILM interpolated video')

    from runway_to_gif import good_gif
    gif_out_path = mp4_out_path.with_suffix('.gif')
    good_gif(mp4_out_path, gif_out_path)
